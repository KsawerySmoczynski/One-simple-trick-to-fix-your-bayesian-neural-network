...
# exapmle
train_set = MNIST("datasets", train=True, transform=T.Compose([T.ToTensor(), T.Normalize((0.1307,), (0.3081,))]), download=True)
test_set = MNIST("datasets", train=False, transform=T.Compose([T.ToTensor(), T.Normalize((0.1307,), (0.3081,))]), download=True)
net_args = {'in_size': 28 * 28, 'hidden_size': 1000, 'out_size': 10}
model_args = {'mean': d(0.), 'std': d(1.)}

run_training(x = train_set, y = test_set, activation = 'relu', net_model = 'mle_classify', net_args = net_args, model_args = model_args, epochs = 15, b_size = 128)
